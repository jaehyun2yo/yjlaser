...
최신 코드 기준(2026-07-09): 프론트엔드는 Next.js 15.5.x/React 19, 백엔드는 `webhard-api`의 NestJS 11/Prisma 6이다. 신규 자체 웹하드 파일은 Google Drive + PostgreSQL 메타데이터를 사용하고, Cloudflare R2는 포트폴리오와 레거시 파일 호환 용도로 남아 있다. LGU+ 외부웹하드 동기화는 이 저장소 내부 CLI가 아니라 루트의 `../외부웹하드동기화프로그램` Electron 앱(v1.5.40)이 담당한다.
...
| **외부웹하드 동기화 앱** | `../외부웹하드동기화프로그램`          | LGU+ 외부웹하드 파일을 자체 웹하드로 단방향 수집하는 Electron 앱     |
...
- 파일 업로드/다운로드 (신규 파일은 Google Drive 업로드/다운로드 프록시, 레거시는 R2 reference 호환)
- 폴더 계층 구조 관리 (무한 중첩)
- 파일 검색 (이름, MIME 타입 기반)
- 휴지통 (소프트 삭제 + 복원)
- 파일 공유 (토큰 기반 공유 링크)
...
│   - Edge Functions       │              │   - storage (Drive/R2)   │
│                          │              │   - trash (휴지통)        │
│   주요 테이블:            │              │   - search (검색)        │
...
┌──────────────────────────────────────────────────────────────────────┐
│   LGU+ 웹하드 동기화 앱 (../외부웹하드동기화프로그램)                   │
│                                                                      │
│   실행 환경: Windows Electron/NSIS 앱                                  │
│                                                                      │
│   ┌──────────┐    ┌──────────────┐    ┌──────────────┐               │
...
NestJS 11 기반의 REST API 서버. 웹하드 파일/폴더 관리와 ERP 기능을 담당한다.
...
- **파일 저장**: 신규 웹하드 파일은 Google Drive 업로드/다운로드 프록시, R2는 포트폴리오/레거시 호환
- **Validation**: class-validator + class-transformer (ValidationPipe)

### 외부웹하드 동기화 앱

LGU+ 웹하드 수집은 루트의 `../외부웹하드동기화프로그램` Electron 앱이 담당한다.

- **아키텍처**: Electron + React + TypeScript + SQLite WAL
- **외부 경계**: Playwright/HTTP 세션으로 LGU+ 파일 감지와 다운로드
- **자체 웹하드 업로드**: NestJS API + Google Drive 저장소
- **배포**: NSIS 설치파일, NAS 자동 업데이트, GitHub Release 폴백
...
| 분류       | 기술                                | 버전 | 용도                                           |
| ---------- | ----------------------------------- | ---- | ---------------------------------------------- |
| 프레임워크 | NestJS                              | 11.x | REST API 서버                                  |
| ORM        | Prisma Client                       | 6.x  | PostgreSQL 스키마 관리, 쿼리 빌더              |
| 인증       | Passport                            | 10.x | 인증 전략 관리                                 |
| 파일 저장  | googleapis, AWS SDK v3              | -    | Google Drive 신규 웹하드, R2 포트폴리오/레거시 |
...
| 분류            | 기술                     | 버전/원천                                  | 용도                             |
| --------------- | ------------------------ | ------------------------------------------ | -------------------------------- |
| 앱 런타임       | Electron + React         | `../외부웹하드동기화프로그램/package.json` | Windows 데스크톱 UI              |
| 언어            | TypeScript               | 5.x                                        | 동기화 로직/렌더러               |
| 브라우저 자동화 | Playwright/HTTP 세션     | -                                          | LGU+ 로그인/업로드 이력/다운로드 |
| DB              | SQLite WAL               | -                                          | 이벤트, 체크포인트, DLQ          |
| 배포            | NSIS + NAS 자동 업데이트 | v1.5.40                                    | 운영 PC 설치/업데이트            |
...
| 파일 저장소     | Google Drive + Cloudflare R2 | 신규 웹하드 파일 + 포트폴리오/레거시 호환 |
...
- **프론트엔드(루트)**: pnpm
- **webhard-api**: pnpm
- **외부웹하드동기화프로그램**: npm/Electron (`../외부웹하드동기화프로그램`)
...
## 7. 파일 저장소 구조 (Google Drive + R2)

신규 자체 웹하드 파일은 Google Drive Shared Drive에 저장하고, PostgreSQL/Prisma의 `webhard_files`, `webhard_folders` 메타데이터와 연결한다. Cloudflare R2는 포트폴리오 이미지와 기존 R2 참조 호환, 레거시 파일 처리에 사용한다.

### 저장소 역할

| 저장소        | 현재 역할                                                      | 주요 코드                                                                                                           |
| ------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Google Drive  | 신규 자체 웹하드 파일/폴더, 거래처별 Drive folder provisioning | `webhard-api/src/storage/google-drive-storage.provider.ts`, `webhard-api/src/folders/drive-provisioning.service.ts` |
| PostgreSQL    | 파일/폴더 메타데이터, 권한, 삭제/이동/다운로드 상태            | `webhard-api/prisma/schema.prisma`                                                                                  |
| Cloudflare R2 | 포트폴리오/레거시 파일 호환, 기존 R2 storage reference 처리    | `src/lib/r2/`, `webhard-api/src/storage/storage.service.ts`                                                         |

### 신규 웹하드 업로드 흐름

```
1. 클라이언트: 업로드 요청 (파일 메타데이터, folderId)
2. NestJS API: Google Drive resumable upload session 생성
3. 클라이언트: `/api/v1/files/google-drive/upload` 프록시로 파일 바이트 전송
4. 클라이언트: 업로드 완료 확인 API 호출
5. NestJS API: Drive file id와 메타데이터를 PostgreSQL에 저장
```
...
2. NestJS API: storageProvider가 GOOGLE_DRIVE이면 Drive stream, R2이면 legacy reference 방식 사용
3. NestJS API: 다운로드 카운트/상태를 메타데이터에 반영
```
...
- Google Drive Provider: `webhard-api/src/storage/google-drive-storage.provider.ts`
- NestJS Storage: `webhard-api/src/storage/storage.service.ts`
- Drive 업로드 프록시: `webhard-api/src/files/files.controller.ts`
- R2 유틸리티: `src/lib/r2/` (포트폴리오/레거시)

---

## 8. LGU+ 웹하드 동기화 설명

LGU+ 외부웹하드 동기화는 이 프로젝트 내부 CLI가 아니라 루트의 `../외부웹하드동기화프로그램` Electron 앱이 담당한다. 현재 코드 기준은 v1.5.40이며, LGU+ 외부웹하드의 `올리기전용` 파일을 감지/다운로드한 뒤 자체 웹하드 API로 업로드하는 단방향 수집 계층이다.

### 동기화 아키텍처

```
┌─────────────────┐          ┌──────────────────┐          ┌──────────────────┐
│  LGU+ 웹하드     │          │  Electron 동기화 앱 │          │  자체 웹하드       │
│  (외부 서비스)    │─────────►│  (외부 프로젝트)  │────────►│  (NestJS + Drive)│
│                 │  Cookie   │                  │  REST    │                  │
│  - 파일 목록     │  기반     │  - History폴링    │  API     │  - Drive 업로드   │
│  - 파일 다운로드  │  HTTP     │  - DLQ/재시도     │          │  - DB 기록        │
...
| 컴포넌트           | 파일 위치                                                                 | 역할                                               |
| ------------------ | ------------------------------------------------------------------------- | -------------------------------------------------- |
| LGU+ client        | `../외부웹하드동기화프로그램/src/core/lguplus-client.ts`                  | 로그인, 세션 복구, 업로드 이력/다운로드 URL 처리   |
| Upload history UI  | `../외부웹하드동기화프로그램/src/renderer/pages/CompletedUploadsPage.tsx` | 업로드 완료 목록, 오늘 표시, 무한스크롤, 태그 필터 |
| 자체 웹하드 client | `../외부웹하드동기화프로그램/src/core`                                    | NestJS API 업로드/확인, DLQ/재시도 연계            |
| 동기화 저장소      | `../외부웹하드동기화프로그램/src`                                         | SQLite WAL 기반 이벤트/체크포인트/DLQ 관리         |

### 동기화 흐름

```
1. Electron 앱이 LGU+ 웹하드 세션을 확보한다.
2. 업로드 이력을 폴링해 신규 파일을 감지한다.
3. 감지 파일을 다운로드하고 자체 웹하드 API에 업로드한다.
4. NestJS API가 Google Drive 저장소와 PostgreSQL 메타데이터를 갱신한다.
5. 실패 건은 SQLite/DLQ와 운영 로그에 남긴다.
6. v1.5.40 기준 실제 운영 설치 앱 장시간 검증은 아직 남아 있다.
```

### 실행 방법

실행/배포 방법은 `../외부웹하드동기화프로그램/README.md`, `PROJECT_STATUS.md`, `CHANGELOG.md`를 기준으로 한다. 이 프로젝트 README에서는 웹 API와 저장소 계약만 설명한다.

### 모니터링

- **Electron UI**: 연결 상태, 감지 상태, 업로드 완료 목록, 업데이트 정보
- **운영 로그/DLQ**: 다운로드/업로드 실패와 재시도 상태
- **웹 API 로그**: `webhard-api`의 파일 업로드/confirm/Drive 경계 로그
...
- **외부웹하드동기화프로그램**: 별도 프로젝트의 `package.json`과 README 기준
...
LGU+ 동기화 앱은 이 프로젝트 내부 서브패키지가 아니다. 실행/배포는 `../외부웹하드동기화프로그램/README.md`, `PROJECT_STATUS.md`, `CHANGELOG.md`를 따른다.
...
GOOGLE_DRIVE_SHARED_DRIVE_ID=     # 신규 웹하드 Shared Drive ID

# ===== Cloudflare R2 (portfolio/legacy) =====
...
CORS_ORIGINS=http://localhost:3000 # CORS 허용 오리진(쉼표 구분)
...
# ===== 외부웹하드 동기화 =====
# LGU+ 자격증명과 동기화 앱 설정은 ../외부웹하드동기화프로그램 쪽 설정을 따른다.
```
...
NestJS API는 Railway Docker 백엔드로 배포한다.
...
- Railway 서비스 환경변수와 Doppler sync 상태 확인
- `CORS_ORIGINS`을 프로덕션 도메인으로 설정
- `DATABASE_URL`에 connection pooling URL 사용

### 11.3 LGU+ 동기화 앱

Windows 운영 PC에는 `../외부웹하드동기화프로그램`의 NSIS 설치본과 NAS 자동 업데이트 경로를 사용한다. 이 웹 프로젝트의 배포 절차에는 포함하지 않는다.
...
| 파일 저장소          | Google Drive + Cloudflare R2 | Drive/R2 대시보드       |
...
현재 웹 프로젝트와 외부 동기화 앱은 다음과 같이 연동되어 있다:
...
Next.js (프론트엔드 + BFF/API)
    ...
    │       ├── Google Drive (신규 웹하드 파일)
    │       └── Cloudflare R2 (포트폴리오/레거시)
    └── Google Drive 업로드/다운로드 프록시

../외부웹하드동기화프로그램 (Electron 앱)
...
- **파일 업로드**: 신규 웹하드 파일은 Google Drive upload session/proxy, 레거시는 R2 reference
- **ERP 작업 관리**: 작업 CRUD, 상태 변경, 작업자/기계 관리

**Next.js API Routes (`/api/`)**

- **인증**: 세션 쿠키 또는 API 키
- **동기화 제어**: LGU+ 동기화 제어는 `../외부웹하드동기화프로그램`에서 담당
...
- Google Drive provider와 R2 reference 유틸을 분리해 저장소별 처리 경계를 유지
- `../외부웹하드동기화프로그램`의 수집 구조를 확장하여 다른 외부 웹하드 서비스 동기화 가능
- NestJS의 `StorageService`/provider 경계를 확장하여 다중 저장소 백엔드 지원
...
- **CORS**: NestJS의 `CORS_ORIGINS` 설정에 연동 시스템의 도메인 추가 필요
...
│   │   ├── files/                        # 파일 모듈
│   │   ├── folders/                      # 폴더 모듈
│   │   ├── storage/                      # 저장소 모듈 (Google Drive + R2)
│   │   ├── trash/                        # 휴지통 모듈
│   │   ├── search/                       # 검색 모듈
│   │   ├── settings/                     # 설정 모듈
│   │   ├── health/                       # 헬스 체크
...
│
├── ../외부웹하드동기화프로그램/         # LGU+ 동기화 Electron 앱 (별도 프로젝트)
│
...
_대상 시스템: Next.js 15.5.x + NestJS 11.x + Prisma 6.x + 외부웹하드동기화프로그램 v1.5.40_
