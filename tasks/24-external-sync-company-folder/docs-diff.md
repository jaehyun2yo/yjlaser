# docs-diff: external-sync-company-folder
...
index 289e3c7c..ac57c8b2 100644
--- a/docs/specs/api/endpoints/integration.md
+++ b/docs/specs/api/endpoints/integration.md
@@ -1395,6 +1395,14 @@ DXF 파일 파싱 결과로 Contact + Order를 자동 생성합니다. 사무실
...
```diff
diff --git a/docs/specs/db/prisma-tables.md b/docs/specs/db/prisma-tables.md
index d9193fba..b49c165d 100644
--- a/docs/specs/db/prisma-tables.md
+++ b/docs/specs/db/prisma-tables.md
@@ -7,15 +7,42 @@ ORM boundary: These tables are accessed ONLY via NestJS (Prisma). Never query fr
...
-| updated_at   | DateTime       |       |
-
-Relations: → webhard_files, webhard_folders
+| Column                          | Type           | Notes                                                          |
+| ------------------------------- | -------------- | -------------------------------------------------------------- |
+| id                              | Int (PK, auto) |                                                                |
+| company_name                    | String         |                                                                |
...
+| approved_by                     | String?        |                                                                |
+
+Indexes: username, company_name, business_registration_number, status, is_approved, created_at DESC
+
+Relations: → webhard_files, webhard_folders, laser_only_mappings, company_folder_aliases

 ### webhard_files

@@ -83,6 +110,24 @@ Maps webhard folder names to laser-only workflow. Primary source for laser-only
...
index 8e803519..ae18b8c5 100644
--- a/docs/specs/features/contact-webhard-folder.md
+++ b/docs/specs/features/contact-webhard-folder.md
@@ -101,6 +101,12 @@ throw 시 `$transaction` 롤백을 유도하여 processStage 전환 자체가
...
```diff
diff --git a/docs/specs/features/external-sync-company-folder.md b/docs/specs/features/external-sync-company-folder.md
new file mode 100644
index 00000000..4dcfaf85
--- /dev/null
+++ b/docs/specs/features/external-sync-company-folder.md
@@ -0,0 +1,146 @@
...
+## 정책 — admin 승인
+
+- 승인 endpoint: `POST /api/v1/companies/folder-aliases/:id/approve` (`AdminAuthGuard`).
...
+- 거절은 `PATCH /api/v1/companies/folder-aliases/:id/reject` 로 단순 status 전환.
...
+  - 쿼리: `status` (선택, 미지정 시 전체), `page` (기본 1), `pageSize` (기본 20).
+  - 응답: `{ items: AliasDto[], total, page, pageSize }`.
+  - `AliasDto` = `{ id, folderName, company: { id, companyName }, status, approvedBy?, approvedAt?, createdAt, updatedAt }`.
+- `POST /api/v1/companies/folder-aliases/:id/approve`
+  - body: `{ cascadeBackfill?: boolean }` (default `false`).
+  - 동작: 대상 alias `status='approved'` + `approvedBy` / `approvedAt` 기록. 동일 `folderName` 의 다른 `pending` 자동 `rejected`. `cascadeBackfill` 시 `ContactFolderSyncService.relocateAfterAliasApproved(folderName, companyId)` 호출.
+  - 응답: 업데이트된 `AliasDto` + `backfilled: number` (cascadeBackfill 시 통합된 Contact 수, false 면 0).
+- `PATCH /api/v1/companies/folder-aliases/:id/reject`
+  - body 없음. `status='rejected'` 로 전환.
+- `DELETE /api/v1/companies/folder-aliases/:id`
+  - row 삭제 (soft delete 아님 — admin 의 의도적 정리).
+
+## Frontend (Phase 4)
+
+- 신규 페이지: `/admin/integration/folder-aliases` (route group `(admin)/admin/integration/folder-aliases/page.tsx`).
+- `IntegrationNav` 에 "폴더 매핑" 탭 1개 추가.
+- 두 패널로 구성:
+  - **`PendingAliasesPanel`**: `status=pending` 후보 list. 각 행에 폴더명 + 매칭된 업체명 + "승인 / 거절" 버튼. 승인 시 `cascadeBackfill` 토글 (default off) 표시. 토글 켜고 승인하면 backfill 결과 (`backfilled: N`) toast.
...
+- **C1** — `relocateAfterAliasApproved(folderName, companyId)`: 해당 folderName 으로 동기화돼 외부웹하드 원본 폴더에 머물던 Contact 들을 모두 매칭된 업체의 `{업체}/문의/...` 로 이동. 반환 `{ relocated: number }`.
+- **C2** — `relocateAfterAliasApproved` 가 `inquiryType=null` Contact 는 skip (현행 미분류 정책 유지).
+- **C3** — `relocateAfterAliasApproved` 가 이미 매칭된 업체 폴더에 있는 Contact 는 멱등 (no-op, relocated 카운트에 포함 안 됨).
+
+### Phase 3 — `folder-alias.service.spec.ts`
+
+- **B1** — `GET ?status=pending`: pending row 만 반환, page/pageSize 적용.
...
+- **B5** — `POST :id/approve { cascadeBackfill: true }`: `relocateAfterAliasApproved` 호출 + `backfilled` 카운트 응답.
+- **B6** — `PATCH :id/reject`: status `rejected`, approvedBy/At 변동 없음.
+- **B7** — `DELETE :id`: row 삭제. 비존재 id → 404.
+
+### Phase 4 — UI (선택)
+
...
+## 참조
+
+- `webhard-api/src/integration/orders/auto-contact.service.ts` — `matchCompanyInfo` (line 156-186, task 23 hotfix). 본 task 의 0차/3차 단계가 추가될 위치.
+- `webhard-api/src/contacts/contact-folder-sync.service.ts` — 단일 진입점 hook 서비스. `relocateAfterAliasApproved` 신규 메서드가 추가될 위치.
+- `webhard-api/src/folders/_lib/company-name-match.util.ts` — `normalizeCompanyName(name)` 정규화 유틸 (task 21). 3차 단계에서 그대로 재사용.
+- `webhard-api/src/companies/companies.controller.ts` — Phase 3 의 `folder-aliases` endpoint 추가 위치 (또는 별도 `folder-aliases.controller.ts` 신설).
+- `webhard-api/src/companies/folder-alias.service.ts` — Phase 3 신규 서비스 (alias CRUD + cascadeBackfill orchestration).
+- `webhard-api/prisma/schema.prisma` `Company` (line 11-66), `LaserOnlyMapping` (line 951-963) — Phase 1 의 `CompanyFolderAlias` 모델 패턴 (relation, unique, index, @@map) 의 레퍼런스.
