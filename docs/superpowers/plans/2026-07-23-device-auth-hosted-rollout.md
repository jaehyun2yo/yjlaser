...
- [ ] **Step 2: 최소 startup attestation 구현**
...
- [ ] **Step 4: attestation 변경만 커밋·push하고 CI 확인**
...
- [ ] **Step 3: staging secret source와 운영 재사용 교정**
...
if (
  (Get-SupabaseProjectRef $developmentSecrets.DATABASE_URL) -ne 'fbtkoikwsytoamlddpms' -or
  (Get-SupabaseProjectRef $developmentSecrets.DIRECT_URL) -ne 'fbtkoikwsytoamlddpms'
) {
  throw 'Doppler dev database가 확인된 yjlaser-dev Supabase project와 다릅니다.'
}
...
- [ ] **Step 4: shared Redis와 환경별 secret 경계 검증**
...
```powershell
$previewDeployment = vercel.cmd inspect $previewUrl --wait --timeout 10m --format json |
  ConvertFrom-Json
