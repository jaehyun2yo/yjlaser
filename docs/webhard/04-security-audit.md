...
**현재 코드**:
```typescript
this.logger.debug(`Auth check: cookieHeader=${!!cookieHeader}, hasCookies=${hasCookies}, hasSessionCookie=${!!sessionCookie}`);

if (cookieHeader && !sessionCookie) {
  this.logger.debug(`Cookie header present but session not found. Available cookies: ${Object.keys(request.cookies || {}).join(', ')}`);
}
...
| Soft Delete 패턴 | 파일/폴더 물리 삭제 방지, 3일 보존 |
...
const ids = folderIds.split(',')
  .map(id => id.trim())
  .filter(id => /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id));
