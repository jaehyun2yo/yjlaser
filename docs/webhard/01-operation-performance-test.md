...
| DELETE | `/trash/:id`         | 승인 body 필요. 휴지통 파일 영구 삭제 | Session |
| DELETE | `/trash`             | 승인 body 필요. 휴지통 비우기         | Session |
...
Client → DELETE /trash/:id
       → Body: { confirmPermanentDelete: true, confirmationText: "PERMANENT_DELETE" }
       → DB: findUnique
       → R2/Drive: deleteFile (Drive는 trashed=true 확인 후 files.delete)
       → DB: delete (레코드 완전 삭제)
```
...
Client → DELETE /trash
       → Body: { confirmPermanentDelete: true, confirmationText: "PERMANENT_DELETE" }
       → DB: findMany (휴지통 파일)
       → R2/Drive: deleteFiles/deleteFile
       → DB: deleteMany
```
