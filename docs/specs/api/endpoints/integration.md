...
| 필드               | 타입            | Required | 설명                                                         |
| ------------------ | --------------- | -------- | ------------------------------------------------------------ |
| idempotency_key    | string          | Y        | 재전송 중복 판단 key. 1차 구현에서는 저장하지 않고 수신만 함 |
...
- `google_drive` 요청은 기존 confirm 경계에서 Drive metadata와 대상 folder readiness를
  확인합니다.
- `r2_legacy`, `local_test`는 내부 confirm provider `r2`로 변환합니다.
- 2026-06-22 기준 `duplicate=false`만 반환합니다. `idempotency_key` 중복 재전송
  처리는 후속 티켓에서 추가합니다.
