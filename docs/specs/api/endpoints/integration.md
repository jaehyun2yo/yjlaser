...
`POST /integration/log-events`만 별도 HMAC 로그 수집 인증을 사용한다.
...
| IBK 알림 트래커 앱          | `POST /integration/bank-notifications`                                                                                                                                                                                                                                                                                                                                                                                   |
| yjlaser_website (관리자)    | `POST /integration/api-keys`, `GET /integration/api-keys`, `DELETE /integration/api-keys/:id`, `GET /integration/sync-logs/pipeline-backlog`, `GET /integration/operations/failures`, `GET /integration/operations/heartbeats`                                                                                                                                                                                           |
| 전체 로깅 클라이언트        | `POST /integration/log-events` — HMAC 서명 기반 중앙 로그 수집 API. `X-API-Key`가 아니라 `X-Log-*` 헤더를 사용한다.                                                                                                                                                                                                                                                                                                      |

---

## Bank Notifications

### POST /api/v1/integration/bank-notifications

IBK 알림 트래커 앱이 수집한 은행 알림 원문과 파싱 필드를 저장합니다.
`CODEX-PROD-*`, `CODEX-DEV-*`, `CODEX-TEST-*` 테스트 마커가 원문 또는
`raw_payload`에 포함되면 서버는 이벤트를 저장하지 않고
`ignored_test_notification`으로 응답하며, 기존 테스트 마커 row 정리를 시도합니다.

### DELETE /api/v1/integration/bank-notifications/test-notifications

기존 은행 알림 이벤트 중 테스트 마커가 포함된 row를 삭제합니다.

**권한:** `bank-notification/manage`
...
### POST /api/v1/integration/log-events

YJLaser 공통 로깅 계약의 구조화 이벤트를 중앙 수집 API로 전송합니다.
이 엔드포인트는 기존 `X-API-Key` 연동 인증을 사용하지 않고 별도 HMAC 서명 인증을
사용합니다. 운영 런타임은 `log_events` Prisma table에 저장하며, 테스트 환경 또는
`LOG_EVENT_PERSISTENCE=memory` 설정에서는 in-memory repository를 사용합니다.

**사용 프로그램:** 모든 로깅 클라이언트 (회사사이트, 외부웹하드동기화프로그램,
관리프로그램, 레이저네스팅프로그램, computeroff)

**Required Headers:**

| 헤더              | 설명                                                                              |
| ----------------- | --------------------------------------------------------------------------------- |
| `X-Log-Client-Id` | 로그 전송 클라이언트 ID                                                           |
| `X-Log-Key-Id`    | 사용 중인 HMAC key ID                                                             |
| `X-Log-Timestamp` | ISO timestamp. 서버 허용 clock skew 기본 5분                                      |
| `X-Log-Nonce`     | 재전송 공격 방지용 1회 nonce                                                      |
| `X-Log-Signature` | `base64url(hmac_sha256(secret, timestamp + "." + nonce + "." + sha256(rawBody)))` |

**Request Body:**

| 필드   | 타입       | Required | 설명                                     |
| ------ | ---------- | -------- | ---------------------------------------- |
| events | LogEvent[] | Y        | 최대 100개. 전체 raw body 제한은 256 KiB |

`LogEvent`는 공통 로깅 계약 v1 필드를 사용합니다. 필수 필드는
`schema_version`, `event_id`, `timestamp`, `level`, `project`, `component`,
`feature`, `event`, `action`, `status`, `channel`, `correlation_id`입니다.

**보안 정책:**

- 요청 본문은 DTO 검증 전에 raw sensitive scanner로 검사한다.
- 비밀번호, 토큰, API key, authorization/cookie, 이메일, 전화번호, presigned URL,
  로컬/UNC 전체 경로, 과도하게 깊은 metadata는 거부한다.
- 인증 실패는 payload 검사보다 먼저 처리되어 `401`을 반환한다.
- 유효한 클라이언트가 허용되지 않은 `project`를 전송하면 `403`을 반환한다.
- 재사용 nonce 또는 충돌하는 `event_id`는 `409`를 반환한다.
- 배치 초과는 `413`, client/IP quota 초과는 `429`, raw sensitive payload 또는 DTO
  검증 실패는 `400`을 반환한다.
- request body, 원본 header 값, raw sensitive match 값은 로그나 응답에 남기지 않는다.
- 저장소에는 원본 `client_id`/`key_id`를 남기지 않고 HMAC short hash와
  server-calculated `payload_hash`만 저장한다.
- 채널별 보관 만료일을 `retention_expires_at`에 저장한다. 실제 삭제 job과 조회 UI는
  별도 운영 작업이다.
...
```json
{
  ...
  "rejected": 0,
  "conflict": 0,
  "results": [{ "event_id": "evt-20260622-100000-abcdef", "status": "accepted" }]
}
```

**관련 상세 스펙:** `docs/specs/features/central-log-ingestion.md`
...
**권한:** `event/write` 또는 legacy `all` 권한을 가진 `X-API-Key`만 허용합니다.
admin/company/worker 세션은 이 쓰기 endpoint를 사용할 수 없습니다.
...
| hostname     | string | N        | legacy wire 호환을 위해 수신하지만 새 row에 저장하지 않음    |
| metadata     | object | N        | legacy wire 호환을 위해 수신하지만 새 row에 저장하지 않음    |

`hostname`과 `metadata`는 기존 클라이언트가 전송해도 DTO 검증에서 거부하지 않지만,
서버는 새 heartbeat row에 저장하거나 응답에 반영하지 않습니다. 기존 DB row의 두 필드는
이 endpoint가 수정·삭제하지 않으며, 정리는 별도 운영 승인 작업입니다.
...
등록된 모든 프로그램 목록을 조회합니다. `lastSeenAt` 기준 120초 미만은 `online`,
그 외는 `offline`으로 표시됩니다.

**사용 프로그램:** yjlaser_website (관리자 대시보드)

**권한:** admin 세션 또는 `operation/read`/legacy `all` 권한을 가진 `X-API-Key`만
허용합니다. company/worker 세션과 다른 integration API key는 거부합니다.

목록 조회는 `id`, 프로그램/인스턴스 식별자, 저장 상태, version, last-seen, created-at만
명시적으로 선택합니다. `hostname`과 `metadata`는 반환하지 않습니다.
...
## Device Authentication (clean RC source only — not deployed)
...
**권한:** admin session만 허용합니다. API key, account recovery key, `Authorization` header는 빈
값과 중복을 포함해 존재만으로 거부합니다.

신규 관리자 session이 등록 코드 발급 또는 장치 승인/폐기 전 CSRF cookie를 준비하는 same-origin
요청입니다. 코드나 자격증명을 만들지 않으며 `Cache-Control: no-store, private`를 반환합니다.

### POST /api/v1/integration/devices/enrollment-codes

**권한:** admin session + 일치하는 CSRF cookie/header만 허용합니다. API key, recovery key,
`Authorization` header는 빈 값과 중복을 포함해 혼용할 수 없습니다.

본문은 `programType`, `capabilityProfile`(`standard` 또는 `safe_canary`), `expectedDisplayName`만
허용합니다. 환경, host/user/hardware/path/metadata는 수집하지 않습니다. 등록 코드는 응답에서
한 번만 반환하며 UI는 브라우저 영속 저장소에 보관하지 않습니다.

### GET /api/v1/integration/devices

**권한:** admin session만 허용합니다. API key, recovery key, `Authorization` header는 빈 값과
중복을 포함해 거부합니다.

서버가 고정한 현재 환경의 장치만 반환합니다. 각 항목은 `deviceId`, `environment`, `programType`,
`capabilityProfile`, `displayName`, 선택 `appVersion`, `state`, `credentialVersion`, `enrolledAt`,
선택 `approvedAt`/`lastHeartbeatAt`/`revokedAt`만 포함합니다. 등록 코드·attempt·refresh/access/
candidate credential·hash·actor·host/user/hardware/path/metadata는 반환하지 않습니다.

### POST /api/v1/integration/devices/:id/approve-enrollment

**권한:** admin session + 일치하는 CSRF cookie/header만 허용합니다. API key, recovery key,
`Authorization` header는 빈 값과 중복을 포함해 거부합니다.

canonical 소문자 UUID `:id`의 `pending_approval` 장치만 승인합니다. request body는 없으며,
`Content-Length`는 없거나 정확히 하나의 `0`만 허용합니다. 그때의 Express 빈 객체 호환 외 payload와
transfer-encoding은 거부합니다. content type은
없거나 단일 `application/json`(parameter 포함)만 허용하며, 다른·중복 content type은 거부합니다.
성공은 HTTP 200이고 `deviceId`, `state`, `environment`, `programType`, `capabilityProfile`,
`credentialVersion`만 반환합니다. 새 등록 코드는 만들지 않습니다.
...
canonical 소문자 UUID `:id`의 pending/active 장치를 폐기합니다. request body 규칙은 승인과 같고,
성공은 HTTP 200 및 장치 목록과 같은 안전 요약을 반환합니다. 소스 transaction은 장치와 준비/활성
refresh credential, 요청/준비 rotation을 종료하도록 구현되어 있습니다. 그러나 token endpoint와
per-request bearer guard는 소스에서 구현되었을 뿐 아직 운영되지 않았습니다. 소스 계약상
heartbeat/canary의 다음 `DeviceBearerGuard` 요청은 폐기를 확인하지만, legacy static-key 업무 호출과
실제 PC 원격 업무 중지를 이 경로가 즉시 차단하지는 않습니다.

세 관리 경로의 정상·거부 응답에는 `Cache-Control: no-store, private`가 적용됩니다. 관리 오류는
`device_management_invalid` (400), `device_management_conflict` (409),
`device_management_unavailable` (503)만 안정 코드로 반환하며 credential 또는 actor 정보를
포함하지 않습니다.

### POST /api/v1/integration/device-auth/enroll

### POST /api/v1/integration/device-auth/enrollment-status

두 bootstrap route는 cookie-less desktop 요청 전용입니다. query, cookie, authorization/API key,
recovery key, CSRF, Origin/Referer header와 압축 body를 거부하고, 전용 4 KiB JSON parser 및
전용 Upstash rate/replay store를 사용합니다. 성공 응답은 장치 상태 여섯 필드만 포함하며 raw
등록 코드, attempt, refresh credential, access token, hash는 응답·오류·로그에 포함하지 않습니다.

### POST /api/v1/integration/device-auth/token

cookie-less token 교환 경로입니다. 정확히 `deviceId`, `refreshCredential`, `nextRefreshCredential`,
`refreshRequestId` 네 string 필드만 받습니다. query/alias, cookie, Authorization, API/recovery/CSRF/
session header, Origin/Referer, transfer-encoding과 압축 body를 거부하고 전용 4 KiB parser와
rate/replay/동시 요청 lease를 사용합니다.

HTTP 201 성공 응답은 정확히 `deviceId`, `environment`, `programType`, `capabilityProfile`,
`credentialVersion`, `accessToken`, `refreshCredentialAction`이며 현재 action은
`replace_with_candidate`만 반환합니다. 동일 응답이 유실되면 recovery window 안에서 동일한 현재
credential, candidate, request ID로만 재시도합니다. raw proof, digest, 내부 row ID, actor는 응답·오류에
포함하지 않습니다. 안정 오류는 `device_refresh_invalid` (401), `device_refresh_in_progress` (409),
`device_revoked` (401), `device_auth_rate_limited` (429), `device_auth_unavailable` (503)입니다.

### POST /api/v1/integration/devices/heartbeat

정확히 하나의 `Authorization: Bearer <device access token>`만 허용합니다. cookie, static API key,
recovery/CSRF/session credential이나 query/alias와 혼용할 수 없습니다. body는 `{}` 또는 canonical
SemVer `appVersion` 한 필드만 허용하며 장치·환경·hostname·path·credential·중첩 metadata는 받지
않습니다. 성공은 HTTP 200 및 `ok`, `deviceId`, `environment`, `programType`, `capabilityProfile`,
`credentialVersion`만 반환합니다.

bearer 검증 후 DB write 전 장치별 6회/60초 quota를 적용합니다. 초과는 429
`device_auth_rate_limited`와 `Retry-After`, rate store 장애는 write 없이 503
`device_auth_unavailable`입니다. 서비스는 현재 환경의 active/non-revoked 장치와 exact credential
version을 CAS 조건으로 `lastHeartbeatAt`과 선택 `appVersion`만 갱신합니다.

### POST /api/v1/integration/devices/canary

heartbeat와 같은 bearer-only/no-ambient 경계를 사용하며 body는 없거나 정확한 `{}`만 허용합니다.
HTTP 200 응답은 `ok`, `contractVersion: "v1"`, `environment`, `programType`, `capabilityProfile`만
반환하고 DB write나 업무 service 호출을 하지 않습니다. `safe_canary`는 heartbeat/canary 외 rotation,
동기화, 발송, DXF, nesting, worker/uploader/reporter/updater 작업을 시작하지 않습니다.

두 bearer 경로는 매 요청마다 선택 환경의 장치 active/revoke 상태, program/profile, credential
version, 활성 credential 만료와 폐기된 token exchange를 DB에서 확인합니다. 폐기는 다음 보호 요청에
`device_revoked` (401)로 반영되지만, 이 보장은 legacy `/integration/programs/heartbeat`나 다른
static-key 업무 경로 및 아직 전환되지 않은 실제 PC로 확장되지 않습니다. 그 밖의 안정 인증 오류는
`device_access_invalid` (401), `device_auth_unavailable` (503)입니다.

공개 rollout 전에는 Railway/edge proxy topology와 client-identity abuse 제어를 별도 승인으로
검증해야 합니다. 상세 계약은 부모 프로젝트의
`docs/contracts/device-auth-v1-desktop-integration.md`를 따릅니다.

### 표준 장치 bearer 업무 endpoint 정책

서버 source의 초기 allowlist는 아래 14개 method/path에만 적용됩니다. 모든 행은 `standard`
capability, 현재 실행 환경, active/non-revoked 장치, 정확한 credential version과 표의 program 및
...
`external_webhard_sync` 장치 bearer는 presign 응답의 `folderId`와 `key`를 변경하지 않고
`POST /files/confirm`에 전달해야 합니다. 장치 confirm은 원래 외부웹하드 husk `folderId`를
재전송할 수 없으며 routing이나 lazy folder 생성을 수행하지 않습니다. 기존 static API key와
admin session의 원본 husk routing fallback은 legacy 호환 경계에만 유지됩니다.

등록되지 않은 장치 bearer route는 default-deny hard hold입니다. 모든 `nesting_program` 업무 route,
file/folder delete·batch delete·multipart/admin mutation, retention, contact/general-contact cleanup,
inventory, nesting task, laser completion, legacy program heartbeat 및 관리자 device/rotation route는
이 초기 allowlist에 포함되지 않습니다. bearer+static key 또는 bearer+named session 혼용도 서비스
호출 전에 거부합니다. 기존 session/static-key 호환은 별도 legacy 정책에만 남아 있으며 bearer
allowlist를 넓히지 않습니다.

남은 범위는 세 데스크톱 vault/client와 fixture 사본, 실제 migration/config, DEV/STG synthetic
검증, 서명 artifact, 배포와 production pilot입니다. 이 문서의 정책은 로컬 source 계약이며 운영
전환 완료를 뜻하지 않습니다.
...
이 엔드포인트는 legacy `OrderEvent` body와 신규 worker event envelope를 모두 받습니다. 신규 envelope는 `idempotency_key`를 포함하며 `JobEvent`에 저장됩니다.
...
**신규 worker event envelope identity fields:**

| 필드            | 타입          | Required | 설명                                                          |
| --------------- | ------------- | -------- | ------------------------------------------------------------- |
| idempotency_key | string        | Y        | Worker 재전송 stable key                                      |
| event_type      | string        | Y        | 도메인 이벤트명                                               |
| event_version   | number        | Y        | payload schema version                                        |
| source_worker   | string        | Y        | `management_program`, `nesting_program` 등                    |
| occurred_at     | ISO string    | Y        | Worker에서 사건이 발생한 시각                                 |
| order_id        | string        | N        | legacy `Order.id` UUID                                        |
| contact_id      | string (UUID) | N        | canonical `Contact.id`. legacy numeric `Order.contactId` 아님 |
| inquiry_number  | string        | N        | 문의번호                                                      |
| work_number     | string        | N        | 현장번호                                                      |
| job_id          | string        | N        | Worker job reference                                          |
| result          | string        | Y        | `success`, `failed`, `partial`                                |
| payload         | object        | Y        | sanitized event payload                                       |
...
- `contact_id`는 Contact UUID입니다. legacy numeric `Order.contactId`는
  `legacy_order_contact_id`로만 반환합니다.
...
### GET /api/v1/integration/operations/failures

해결되지 않은 `JobFailure` 목록을 cursor 기반으로 조회합니다.

**사용 프로그램:** yjlaser_website (관리자 대시보드)

**권한:** 관리자 세션 또는 `operation/read` permission을 가진 `X-API-Key`

**Query Parameters:**

| 필드   | 타입   | Required | 기본값 | 설명                             |
| ------ | ------ | -------- | ------ | -------------------------------- |
| cursor | string | N        | -      | 다음 페이지 기준 `JobFailure.id` |
| limit  | number | N        | 50     | 1-100 범위로 보정                |
  ...
  "items": [
    {
      ...
      "created_at": "2026-06-19T09:03:00.000Z",
      "updated_at": "2026-06-19T09:03:00.000Z",
      "last_event": {
...
- `resolvedAt = null`인 unresolved `JobFailure`만 반환합니다.
- 정렬 기준은 `createdAt desc, id desc`입니다.
- `contact_id`, `inquiry_number`, `work_number`는 raw payload를 열지 않고
  실패 원인을 중앙 Contact와 연결하기 위한 read model 필드입니다.
- `JobFailure.metadata`, `JobEvent.payload`, `JobEvent.idempotencyKey`는 select하지 않고
  응답에도 포함하지 않습니다.
- company session은 service 조회 전 `INTEGRATION_OPERATION_ACCESS_DENIED` 403으로
  거부합니다.
- `OperationsService` start/success/failure 로그에는 `limit`, `hasCursor`, `count`,
  `hasMore`, `elapsedMs`, `errorType`만 남기고 raw cursor와 에러 메시지는 남기지
  않습니다.
...
| ---- | ------------------------------------------ |
| 401  | 유효한 session 또는 `X-API-Key` 없음       |
| 403  | API key에 `operation/read` permission 없음 |

---

### GET /api/v1/integration/operations/heartbeats

`ProgramHeartbeat`를 운영 대시보드용 online/late/offline read model로 조회합니다.
  ...
  "items": [
    {
      ...
      "version": "1.0.0",
      "hostname": "HOST-01",
      "last_seen_at": "2026-06-19T09:03:00.000Z",
      "lag_seconds": 60,
...
- 정렬 기준은 `programType asc, instanceName asc`입니다.
- `lastSeenAt` 기준 120초 초과는 `late`, 10분 초과는 `offline`입니다.
- 저장된 `status = offline`은 `lastSeenAt`과 무관하게 `offline`으로 반환합니다.
- `ProgramHeartbeat.metadata`는 select하지 않고 응답에도 포함하지 않습니다.
- company session은 service 조회 전 `INTEGRATION_OPERATION_ACCESS_DENIED` 403으로
  거부합니다.
- `OperationsService` start/success/failure 로그에는 count, online/late/offline 집계,
  `elapsedMs`, `errorType`만 남기고 instanceName, hostname, metadata, 에러 메시지는
  남기지 않습니다.
