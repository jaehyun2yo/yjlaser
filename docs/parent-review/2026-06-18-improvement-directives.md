...
웹 플랫폼은 기본 기능과 배포 기반은 갖췄지만, 운영 QA와 P0 안정화가 먼저다. 새 기능보다 Worker 상태 변경 오류, 대용량 업로드, 운영 env/migration, realtime/session QA를 우선 정리한다.
...
| YJW-PR-002 | P0 | R2 대용량 DXF 업로드 타임아웃 | 허용 용량, timeout, multipart/presigned 흐름, 사용자 오류 메시지를 점검한다 | 대용량 샘플 업로드 기준과 실패 시 복구/재시도 기준 문서화 | 업로드 smoke 또는 수동 QA 기록 |
