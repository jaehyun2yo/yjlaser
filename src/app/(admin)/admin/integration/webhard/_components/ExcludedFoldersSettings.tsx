...
import { getExcludedFolders, updateExcludedFolders } from '@/app/actions/webhard';
...
export default function ExcludedFoldersSettings() {
  ...
  const saveToDb = async (next: string[]) => {
    if (next.length === 0) {
      showMessage('error', '최소 1개의 제외폴더가 필요합니다.');
      return;
    }
    setSaving(true);
    const result = await updateExcludedFolders(next);
    if (result.success) {
      setFolders(next);
  ...
  return (
    <div className={`${BG_COLOR.card} p-6 rounded-xl shadow-md border ${BORDER_COLOR.default}`}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className={`text-lg font-bold ${TEXT_COLOR.primary}`}>제외폴더 설정</h2>
          <p className={`text-sm ${TEXT_COLOR.secondary} mt-1`}>
            업체명 추출 시 건너뛰는 구조적 폴더입니다. 이 폴더에 직접 업로드된 파일은 미분류로
            처리됩니다.
          </p>
        </div>
