...
import { ApproveButton } from '@/app/(admin)/admin/companies/[id]/approve-button';
import { WebhardAccessToggle } from '@/app/(admin)/admin/companies/[id]/webhard-access-toggle';
import { CompanyDeleteActions } from '@/app/(admin)/admin/companies/[id]/company-delete-actions';
...
export default async function IntegrationCompanyDetailPage({
  params,
}: {
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {company.webhard_access === false && (
            <span
              className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${BG_COLOR.orangeLight} ${TEXT_COLOR.orangeDark}`}
            >
              {' '}
          ...
          <Button variant="ghost" asChild className="flex items-center gap-2">
            <Link href={`/admin/webhard/activity?companyId=${company.id}`}>
              <FaHistory />
              활동 로그
            </Link>
          </Button>
