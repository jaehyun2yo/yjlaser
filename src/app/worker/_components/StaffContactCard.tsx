...
import StaffAdvanceButton from './StaffAdvanceButton';
import { ConfirmModal } from './ConfirmModal';
import { Siren, MessageSquare, Download, ChevronDown, Upload } from 'lucide-react';
...
export default function StaffContactCard({
  ...
  const canUploadDrawing = contact.process_stage === 'drawing_confirmed';
  ...
  return (
    ...
    >
      <div className="px-3 py-2.5">
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              {hasNewContactNotification && (
                <span
                  aria-hidden="true"
                  data-testid={`worker-contact-new-dot-${contact.id}`}
              ...
              {stageInfo && (
                <Badge
                  size="xs"
                  className={`shrink-0 font-medium ${stageInfo.bgColor} ${stageInfo.color}`}
                >
                  {stageInfo.label}
                </Badge>
              )}
        ...
        {/* Fallback: 기존 단일 메모 (worker_notes가 비어있을 때) */}
