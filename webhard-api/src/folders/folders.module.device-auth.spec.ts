...
describe('FoldersModule device authentication wiring', () => {
  it('imports device authentication and registers the composite route guards', () => {
    const imports = Reflect.getMetadata('imports', FoldersModule) as unknown[];
    const providers = Reflect.getMetadata('providers', FoldersModule) as unknown[];
