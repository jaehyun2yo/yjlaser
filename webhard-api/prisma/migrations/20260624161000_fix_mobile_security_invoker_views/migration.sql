...
REVOKE SELECT ON TABLE public.im_mobile_price_update_requests FROM authenticated;
REVOKE SELECT ON TABLE public.im_dxf_files FROM authenticated;
REVOKE SELECT ON TABLE public.im_dxf_previews FROM authenticated;
REVOKE SELECT ON TABLE public.im_mobile_worker_heartbeat FROM authenticated;
...
DROP POLICY IF EXISTS "mobile_unpriced_read" ON public.im_dxf_files;
CREATE POLICY "mobile_unpriced_read" ON public.im_dxf_files
    FOR SELECT TO authenticated
    USING (COALESCE(price, 0) = 0 AND status = 'classified');

DROP POLICY IF EXISTS "mobile_preview_unpriced_read" ON public.im_dxf_previews;
...
DROP POLICY IF EXISTS "heartbeat_status_select" ON public.im_mobile_worker_heartbeat;
